// API service for ASTROSPACESUMMIT2026 website
...
export const fetchContent = (key) => get(`/content/${key}?conference=astro`);

// Get all content blocks at once
export const fetchAllContent = () => get('/content?conference=astro');

// Speakers — uses public endpoint (visible only, sorted by order)
export const fetchSpeakers = (category) =>
    get(`/speakers?conference=astro${category ? `&category=${encodeURIComponent(category)}` : ''}`);

// Sponsors/Media partners — uses public endpoint (visible only)
export const fetchSponsors = (type) =>
    get(`/sponsors?conference=astro${type ? `&type=${encodeURIComponent(type)}` : ''}`);

// Submit an abstract — always tags conference: 'astro'
export async function submitAbstract(payload) {
    const res = await fetch(`${BASE_URL}/abstracts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...payload, conference: 'astro' }),
    });
...
// Submit registration — always tags conference: 'astro'
...
export async function validateDiscountCode(coupon) {
    try {
        const res = await fetch(`${BASE_URL}/discounts/validate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ coupon, conference: 'astro' }),
        });
        if (!res.ok) throw new Error('Server error');
        return res.json();
    } catch (e) {
