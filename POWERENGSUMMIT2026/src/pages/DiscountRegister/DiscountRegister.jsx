...
const getActivePhase = () => {
    const now = new Date();
    if (now <= new Date('2026-11-25')) return 'early';
    if (now <= new Date('2027-01-25')) return 'standard';
    return 'onspot';
};
...
const DiscountRegister = () => {
    ...
    const handleValidateCoupon = useCallback(async () => {
        ...
        try {
            const result = await siteApi.validateDiscount(code);
            if (result.valid) {
            ...
            setDiscount(null); setCouponStatus('invalid');
            setCouponMsg('Could not verify code. Please try again.');
        }
    }, [couponInput]);
    ...
    const handleSubmit = async (e) => {
        ...
        const selectedCat = pricingData.find(p => p.id === selectedCategory);
        const selectedSpon = sponsorshipPricing.find(p => p.id === selectedSponsorship);

        const descParts = [];
        if (selectedCat) descParts.push(`${selectedCat.label} : $${selectedCat[activePhase]}`);
        if (selectedSpon) descParts.push(`${selectedSpon.label} : $${selectedSpon.price}`);
        ...
        try {
            ...
            const { order } = await siteApi.createPaymentOrder({
                ...
                description: `PowerEng Discount Reg: ${formData.fullName}`
            });
            ...
            const options = {
                ...
                description: `Discount Registration for ${formData.fullName}`,
                order_id: order.id,
        ...
        <div className="discount-register-page">
            ...
            <div className="container section-padding">
                <div className="discount-register-container">
                    ...
                    <form className="discount-register-form" onSubmit={handleSubmit}>
                        ...
                        <table className="or-pricing-table">
                            <thead>
                                <tr>
                                    <th>Category</th>
                                    <th className={activePhase === 'early' ? 'or-active-col' : ''}>Early Bird<br /><span className="or-date">Nov 25, 2026</span>{activePhase === 'early' && <span className="or-active-badge">ACTIVE</span>}</th>
                                    <th className={activePhase === 'standard' ? 'or-active-col' : ''}>Standard<br /><span className="or-date">Jan 25, 2027</span>{activePhase === 'standard' && <span className="or-active-badge">ACTIVE</span>}</th>
                                    <th className={activePhase === 'onspot' ? 'or-active-col' : ''}>On-Spot<br /><span className="or-date">Mar 23, 2027</span>{activePhase === 'onspot' && <span className="or-active-badge">ACTIVE</span>}</th>
                                </tr>
                            </thead>
                            <tbody>
