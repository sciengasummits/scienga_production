// API service for HEALTHMEDSUMMIT2026 / GLOHEALTHSUMMIT2026 website
// Fetches live data from the sciengasummits_workflow dashboard backend
...
export const fetchContent = (key) => get(`/content/${key}?conference=healthmed`);

// Get all content blocks at once
export const fetchAllContent = () => get('/content?conference=healthmed');

// Speakers — uses public endpoint (visible only, sorted by order)
export const fetchSpeakers = (category) =>
    get(`/speakers?conference=healthmed${category ? `&category=${encodeURIComponent(category)}` : ''}`);

// Sponsors/Media partners — uses public endpoint (visible only)
export const fetchSponsors = (type) =>
    get(`/sponsors?conference=healthmed${type ? `&type=${encodeURIComponent(type)}` : ''}`);

// Submit an abstract — always tags conference: 'healthmed'
export async function submitAbstract(payload) {
    const res = await fetch(`${BASE_URL}/abstracts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...payload, conference: 'healthmed' }),
    });
...
// Submit registration — always tags conference: 'healthmed'
...
export async function validateDiscountCode(coupon) {
    try {
        const res = await fetch(`${BASE_URL}/discounts/validate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ coupon, conference: 'healthmed' }),
        });
        if (!res.ok) throw new Error('Server error');
        return res.json();
    } catch (e) {
